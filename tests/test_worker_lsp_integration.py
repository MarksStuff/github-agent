...
"""
            ...
            for i in range(max_retries):
                try:
                    response = requests.get(
                        f"http://localhost:{self.worker_port}/health", timeout=1
                    )
                    if response.status_code == 200:
                        print(
                            f"✅ Worker started successfully on port {self.worker_port}"
                        )
                        ...
                        if not self.tools_ready:
                            print(
                                "⚠️ Tools not ready, getting worker output for debugging..."
                            )
    ...
    def call_worker_tool(self, tool_name: str, **kwargs):
        ...
        if response.status_code != 200:
            raise RuntimeError(
                f"Worker request failed: {response.status_code} - {response.text}"
            )
    ...
    def test_worker_lsp_integration(self):
        ...
        try:
            ...
            try:
                health_result = self.call_worker_tool(
            ...
            if not self.tools_ready:
                ...
                if definition_data.get("definitions"):
                    ...
                    print("⚠️  No definitions found, but no timeout")
            ...
            if not self.tools_ready:
                ...
                if references_data.get("references"):
                    ...
                    print("⚠️  No references found, but no timeout")
            ...
            if not self.tools_ready:
                ...
                symbols_result = self.call_worker_tool(
                    "search_symbols", repository_id="test-repo", query="DataProcessor"
                )
