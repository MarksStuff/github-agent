...
class WorkflowInputs:
    ...
    def calculate_checksum(self) -> str:
        ...
        canonical = {
            ...
            'config_overrides': sorted(self.config_overrides.items()) if self.config_overrides else [],
...
class WorkflowState:
    ...
    def has_inputs_changed(self, new_inputs: WorkflowInputs) -> bool:
        ...
        if changed:
            logger.info(f"Inputs changed: {self.inputs_checksum[:8]} -> {new_checksum[:8]}")
        else:
    ...
    def start_stage(self, stage_name: str):
        ...
        if stage_name not in self.stages:
            self.stages[stage_name] = StageState(name=stage_name, status=StageStatus.PENDING)
    ...
    def complete_stage(self, stage_name: str, output_files: List[str] = None, metrics: Dict[str, Any] = None):
    ...
    @classmethod
    def load(cls, workflow_id: str, state_file: Optional[Path] = None) -> Optional['WorkflowState']:
        """
        ...
        try:
            with open(instance.state_file, 'r') as f:
                data = json.load(f)
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        completed_count = len([s for s in self.stages.values() if s.status == StageStatus.COMPLETED])
        failed_count = len([s for s in self.stages.values() if s.status == StageStatus.FAILED])
        ...
        return {
            ...
            'progress_percent': (completed_count / len(self.stages)) * 100 if self.stages else 0,
            'is_complete': self.is_workflow_complete(),
            'can_resume': self.can_resume(),
            'inputs_checksum': self.inputs_checksum[:8] if self.inputs_checksum else None
        }
