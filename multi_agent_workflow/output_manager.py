...
class WorkflowProgressDisplay:
        ...
        for stage_name, stage in state.stages.items():
            ...
            if stage.output_files:
                if len(stage.output_files) <= 2:
                    ...
                    files_text = f"{stage.output_files[0]}, ... (+{len(stage.output_files)-1})"
    ...
    def create_metrics_panel(self, state: WorkflowState) -> Panel:
        ...
        metrics_text.append(f"Progress: {summary['progress_percent']:.1f}% ", style="white")
        metrics_text.append(f"({summary['completed_stages']}/{summary['total_stages']} stages)\n", style="dim")
        metrics_text.append(f"Status: ", style="white")
    ...
    def display_workflow_status(self, state: WorkflowState, project_desc: str = None):
        ...
        # Create header
        header = self.create_workflow_header(state.workflow_id, project_desc or "Multi-Agent Project")
    ...
    def show_stage_complete(self, stage_name: str, result: Dict[str, Any]):
        ...
        if result.get("metrics"):
            ...
            for key, value in metrics.items():
                if key != "duration":
                    summary_text += f"[dim]{key.replace('_', ' ').title()}: {value}[/dim]\n"
    ...
    def show_workflow_complete(self, state: WorkflowState):
        ...
        celebration_text.append(f"Successfully completed all {summary['total_stages']} stages\n", style="white")
        celebration_text.append(f"Total time: {summary['updated_at']} - {summary['created_at']}\n", style="dim")
...
class WorkflowLogger:
    ...
    def stage_start(self, stage_name: str, description: str):
        ...
        stage_display = stage_name.replace("_", " ").title()
        self.logger.info(f"[bold blue]🚀 Starting {stage_display}[/bold blue]: {description}")
    ...
    def stage_complete(self, stage_name: str, duration: float = None):
        ...
        self.logger.info(f"[bold green]✅ Completed {stage_display}[/bold green]{duration_text}")
