...
class RequirementsAnalysisStage(WorkflowStageExecutor):
    ...
    def validate_inputs(self, inputs: WorkflowInputs) -> List[str]:
        ...
        if not inputs.project_description or len(inputs.project_description.strip()) < 10:
...
class TestingSetupStage(WorkflowStageExecutor):
    ...
    def __init__(self):
        super().__init__(
            "testing_setup", 
            "Set up testing framework and create initial test cases"
        )
...
class WorkflowOrchestrator:
    ...
    def start_workflow(self, project_description: str, config_overrides: Optional[Dict[str, Any]] = None, 
                      template_name: Optional[str] = None, from_stage: Optional[str] = None,
                      to_stage: Optional[str] = None) -> str:
        """
    ...
    def resume_workflow(self, workflow_id: str, from_stage: Optional[str] = None,
                       to_stage: Optional[str] = None) -> str:
        """
    ...
    def _execute_workflow(self, state: WorkflowState, inputs: WorkflowInputs,
                         from_stage: Optional[str] = None, to_stage: Optional[str] = None) -> str:
        """
        ...
        for stage_name in stages_to_execute:
            ...
            try:
                ...
                self.logger.info(f"Executing stage: {stage_name} - {stage_executor.description}")
                ...
                if result.get("next_actions"):
                    self.logger.info(f"Next actions: {', '.join(result['next_actions'])}")
  ...
  python workflow.py list
    ...
    try:
        if args.command == "start":
            ...
            print(f"   Progress: {status['progress_percent']:.1f}% ({status['completed_stages']}/{status['total_stages']} stages)")
            ...
            if not workflows:
                ...
                for workflow in workflows:
                    ...
                    print(f"   Progress: {workflow['progress_percent']:.1f}% ({workflow['completed_stages']}/{workflow['total_stages']} stages)")
