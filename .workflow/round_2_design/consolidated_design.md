...
```sql
-- Optional future enhancement - extend existing symbols table
...
2. **Phase 2 - Hierarchy Support** (Backward compatible)
   - Deploy `DocumentSymbolProvider` with cache
   - Extend `Symbol` with optional hierarchy fields
   - Enable hierarchy building from LSP response

3. **Phase 3 - Storage Enhancement** (Optional)
   - Add database columns via migration script
   - Implement persistent hierarchy storage
   - Add cross-file symbol relationship tracking
...
- Database changes are additive (new columns) - no data loss on rollback
