...
class TestGitHubIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_pr_comments_with_since_filter(self, mock_github_class):
        ...
        mock_old_comment.html_url = "https://github.com/test/repo/pull/123#issuecomment-1"
        ...
        mock_new_comment.html_url = "https://github.com/test/repo/pull/123#issuecomment-2"
    ...
    async def test_wait_for_checks(self, mock_github_class):
        ...
        with patch.object(self.github_integration, 'get_ci_status', side_effect=mock_get_ci_status):
            # Execute with short timeout and poll interval for testing
            status = await self.github_integration.wait_for_checks(123, timeout=5, poll_interval=1)
    ...
    async def test_extract_actionable_feedback(self):
        ...
        normal_priority = next(item for item in actionable if item["priority"] == "normal")
