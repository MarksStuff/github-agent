...
""")
            ...
            (repo_path / "README.md").write_text("""# Test API Project
...
""")
            ...
            (src_dir / "auth.py").write_text("""\"\"\"Authentication module.\"\"\"
...
""")
            ...
            (src_dir / "users.py").write_text("""\"\"\"User management module.\"\"\"
...
""")
    ...
    async def test_real_feature_extraction(self, real_workflow, temp_repo):
        ...
        assert result["feature_description"] == "Add OAuth2 authentication with role-based access control"
    ...
    async def test_real_codebase_analysis(self, real_workflow, temp_repo):
        ...
        print(f"🔍 Real analysis found: {len(analysis['languages'])} languages, {len(analysis['key_files'])} key files")
    ...
    async def test_real_ollama_code_context_generation(self, real_workflow, temp_repo):
        ...
        assert "FastAPI" in context_doc or "API" in context_doc or "Python" in context_doc
    ...
    async def test_real_agent_analysis(self, real_workflow, temp_repo):
        ...
        analysis = await real_workflow._agent_analysis(
            real_workflow.agents["senior-engineer"], 
            "senior-engineer",
            agent_context
        )
