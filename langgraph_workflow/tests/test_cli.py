...
class TestInteractiveMode(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_interactive_list_threads(self, mock_sqlite, mock_input):
        ...
        mock_cursor.execute.assert_called_once_with("SELECT DISTINCT thread_id FROM checkpoints")
    ...
    @patch('builtins.input') 
    @patch('langgraph_workflow.run.run_workflow')
    async def test_interactive_missing_required_fields(self, mock_run_workflow, mock_input):
...
class TestMainFunction(unittest.TestCase):
    ...
    @patch('sys.argv', ['run.py', '--repo-path', '/tmp/repo', '--feature', 'Test feature'])
    @patch('langgraph_workflow.run.run_workflow')
    @patch('asyncio.run')
    ...
    @patch('sys.argv', ['run.py', '--repo-path', '/tmp/repo', '--feature-file', 'features.md', '--feature-name', 'Auth'])
    @patch('langgraph_workflow.run.run_workflow')
    @patch('asyncio.run')
    ...
    @patch('sys.argv', ['run.py', '--repo-path', '/tmp/repo', '--thread-id', 'test-123', '--resume'])
    @patch('langgraph_workflow.run.run_workflow')  
    @patch('asyncio.run')
...
class TestArgumentParsing(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.parser.add_argument("--feature-file", help="Path to file containing feature description or PRD")
        self.parser.add_argument("--feature-name", help="Name of specific feature within PRD (requires --feature-file)")
        self.parser.add_argument("--thread-id", help="Thread ID for persistence")
        self.parser.add_argument("--checkpoint-path", default="agent_state.db", help="SQLite checkpoint database path")
        self.parser.add_argument("--resume", action="store_true", help="Resume from existing checkpoint")
        self.parser.add_argument("--interactive", action="store_true", help="Run in interactive mode")
    ...
    def test_parse_custom_checkpoint_path(self):
        ...
        args = self.parser.parse_args([
            "--checkpoint-path", "/custom/path/state.db"
        ])
