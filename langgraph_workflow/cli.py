...
class LangGraphCLI:
    ...
    def __init__(self):
        ...
        self.checkpointer_path = os.getenv("LANGGRAPH_DB_PATH", ".langgraph_checkpoints/agent_state.db")
    ...
    def _detect_repo_name(self) -> str:
        ...
        try:
            ...
            raise ValueError(f"Repository path {self.repo_path} is not a git repository or has no remote")
        except Exception as e:
            logger.error(f"Failed to detect repository name: {e}")
            raise ValueError(f"Could not determine repository name from {self.repo_path}")
    
    def _extract_feature_from_prd(self, prd_file: Path, feature_name: str) -> tuple[str, str]:
...
"""
        ...
        senior_engineer = self.workflow_graph.agent_nodes.orchestrator.agents["senior_engineer"]
...
{result}"""
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Initialize components
            self.workflow_graph = WorkflowGraph(self.repo_name, self.repo_path, self.checkpointer_path)
    ...
    async def cmd_start(self, args):
        ...
        if args.prd_file:
            ...
            try:
                ...
                task_spec, feature_name = await self._extract_feature_from_prd(prd_file, args.feature)
            except Exception as e:
        ...
        thread_id = args.thread_id or f"workflow-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    async def cmd_artifacts(self, args):
        ...
        if args.artifacts_action == "list":
            ...
            exported_path = self.artifact_manager.export_artifacts(args.thread_id, export_path)
            print(f"✅ Exported artifacts to: {exported_path}")
        
        elif args.artifacts_action == "cleanup":
            self.artifact_manager.cleanup_thread(args.thread_id, keep_final_artifacts=args.keep_final)
...
def create_parser():
    ...
    artifacts_parser.add_argument("artifacts_action", 
    ...
    artifacts_parser.add_argument("--keep-final", action="store_true", 
                                help="Keep final artifacts during cleanup")
    
    # Models command
    models_parser = subparsers.add_parser("models", help="Manage model connections")
    models_parser.add_argument("models_action", 
                             choices=["test", "stats"],
                             help="Models action")
