    ...
    async def initialize_git(self, state: WorkflowState) -> dict:
        ...
        try:
            ...
            state["messages_window"].append({
                "role": "git",
                "content": f"Initialized branch {branch_name}"
            })
            ...
            state["messages_window"].append({
                "role": "system",
                "content": f"Git initialization failed: {str(e)}",
                "error": True
            })
...
Co-Authored-By: Claude <noreply@anthropic.com>"""
                    ...
                    state["messages_window"].append({
                        "role": "git",
                        "content": f"Committed changes: {message}"
                    })
                else:
            ...
            state["messages_window"].append({
                "role": "system",
                "content": f"Git commit failed: {str(e)}",
                "error": True
            })
    ...
    async def push_branch_and_pr(self, state: WorkflowState) -> dict:
        ...
        try:
            ...
            if not state.get("pr_number"):
                ...
                state["messages_window"].append({
                    "role": "github",
                    "content": f"Created PR #{pr_number}"
                })
            else:
            ...
            state["messages_window"].append({
                "role": "system",
                "content": f"Push/PR failed: {str(e)}",
                "error": True
            })
    ...
    async def fetch_pr_comments(self, state: WorkflowState) -> dict:
        ...
        try:
            ...
            state["messages_window"].append({
                "role": "system",
                "content": f"PR comment fetch failed: {str(e)}",
                "error": True
            })
