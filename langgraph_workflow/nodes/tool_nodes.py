...
from langgraph_workflow.state import WorkflowState, QualityState
...
class ToolNodes:
    ...
    async def run_tests(self, state: WorkflowState) -> dict:
        ...
        try:
            ...
            result = subprocess.run([
                sys.executable, "-m", "pytest", "-xvs", "--tb=short", "--json-report", 
            ...
            if test_results["passed"]:
                ...
                logger.warning(f"Tests failed with {len(test_results.get('failed_tests', []))} failures")
            ...
            state["messages_window"].append({
                ...
                "details": {
                    ...
                    "total_time": test_results.get("summary", {}).get("duration", 0)
                }
            })
        ...
        try:
            ...
            if result.stdout:
                try:
                    ...
                    lint_findings = [{"message": line} for line in result.stdout.split("\n") if line.strip()]
            
            lint_status = {
                "returncode": result.returncode,
                "findings": lint_findings,
                "error_count": len([f for f in lint_findings if f.get("level") == "error"]),
                "warning_count": len([f for f in lint_findings if f.get("level") == "warning"]),
                "passed": result.returncode == 0,
                "timestamp": datetime.now().isoformat(),
            }
            ...
            logger.info(f"Lint check completed: {lint_status['error_count']} errors, {lint_status['warning_count']} warnings")
    ...
    async def apply_patch(self, state: WorkflowState, patch_content: str, target_file: str) -> dict:
        ...
        try:
            ...
            state["messages_window"].append({
                "role": "system",
                "content": f"Patch application failed: {str(e)}",
                "error": True
            })
    ...
    async def check_ci_status(self, state: WorkflowState) -> dict:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                # Add to messages
                status = "PASSED" if ci_status["all_passed"] else "IN_PROGRESS" if ci_status["in_progress"] else "FAILED"
                state["messages_window"].append({
                    "role": "ci",
                    "content": f"CI status: {status}",
                    "details": {
                        "checks_count": len(checks_data),
                        "passed_count": len([c for c in checks_data if c.get("conclusion") == "success"])
                    }
                })
            ...
            state["messages_window"].append({
                "role": "system",
                "content": f"CI status check failed: {str(e)}",
                "error": True
            })
