...
class ModelRouter:
    ...
    def __init__(self, ollama_host: Optional[str] = None):
        ...
        ollama_url = ollama_host or os.getenv(
        ...
        try:
            self.ollama_client = ChatOllama(
                base_url=ollama_url, model="qwen2.5-coder:7b", temperature=0.1
            )
        ...
        self.escalation_threshold = int(
            os.getenv("MODEL_ROUTER_ESCALATION_THRESHOLD", "2")
        )
    ...
    def should_escalate_to_claude(
        self, state: WorkflowState, agent_type: AgentType
    ) -> bool:
    ...
    async def call_model(
        self, prompt: str, state: WorkflowState, agent_type: AgentType
    ) -> str:
    ...
    async def _call_claude(
        ...
        for attempt in range(max_retries):
            try:
                ...
                contextual_prompt = self._add_context_for_claude(
                    prompt, state, agent_type
                )
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"Claude attempt {attempt + 1} failed: {e}, retrying in {delay}s"
                    )
    ...
    def _add_context_for_claude(
        ...
        if state.get("retry_count", 0) > 0:
            context_parts.append(
                f"This is retry attempt #{state['retry_count']} - previous approaches failed."
            )

        if state.get("design_conflicts"):
            context_parts.append(
                f"There are {len(state['design_conflicts'])} design conflicts to resolve."
            )
    ...
    async def test_connections(self) -> dict:
        ...
        if self.ollama_client:
            try:
                ...
                response = await asyncio.wait_for(
                    self.ollama_client.ainvoke([test_message]), timeout=30
                )
                results["ollama"] = {
                    "status": "connected",
                    "response": str(response.content)[:50]
                    if hasattr(response, "content")
                    else str(response)[:50],
                }
            except Exception as e:
        ...
        if self.claude_client:
            try:
                ...
                response = await asyncio.wait_for(
                    self.claude_client.ainvoke([test_message]), timeout=30
                )
                results["claude"] = {
                    "status": "connected",
                    "response": str(response.content)[:50]
