...
class WorkflowState(TypedDict):
    ...
    messages_window: Annotated[Sequence[BaseMessage], lambda x, y: y[-10:]]  # Keep last 10
    summary_log: str
...
class MultiAgentWorkflow:
    ...
    def __init__(
        ...
        self.thread_id = thread_id or f"session-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow.add_node("parallel_design_exploration", self.parallel_design_exploration)
        ...
        workflow.add_conditional_edges(
            ...
            {
                True: "finalize_design_document",
                False: "iterate_design_document"
            }
        )
...
"""
        ...
        state["messages_window"].append(
            AIMessage(content=f"Extracted code context document (saved to {context_path})")
        )
...
Remain neutral and document rather than judge."""
        ...
        state["messages_window"].append(
            AIMessage(content=f"Synthesis complete. Found {len(conflicts)} conflicts, {len(questions)} questions.")
        )
    ...
    async def code_investigation(self, state: WorkflowState) -> WorkflowState:
        ...
        updated_synthesis = state["synthesis_document"] + "\n\n## Code Investigation Results\n"
        for q, r in zip(questions, investigation_results):
            updated_synthesis += f"\n**Q:** {q}\n**A:** {r}\n"
...
"""
    ...
    async def iterate_design_document(self, state: WorkflowState) -> WorkflowState:
        ...
        contribution = await self._agent_design_contribution(
            agent, agent_type, state["design_document"], state["design_constraints_document"]
        )
    ...
    async def parallel_development(self, state: WorkflowState) -> WorkflowState:
        ...
        impl_task = self._write_implementation(state["skeleton_code"], state["design_document"])
    ...
    async def reconciliation(self, state: WorkflowState) -> WorkflowState:
        ...
        if mismatches:
            ...
            test_argument = await self._agent_argument(
                self.agents[AgentType.TEST_FIRST],
                "tests",
                mismatches
            )
            impl_argument = await self._agent_argument(
                self.agents[AgentType.FAST_CODER],
                "implementation",
                mismatches
            )
            ...
            if not await self._has_consensus(
                ...
                # Apply arbitration
                state["test_code"], state["implementation_code"] = await self._apply_reconciliation(
                    state["test_code"],
                    state["implementation_code"],
                    arbitration
                )
        ...
        state["messages_window"].append(
            AIMessage(content=f"Reconciliation complete. {len(mismatches)} mismatches resolved")
        )
    ...
    async def component_tests(self, state: WorkflowState) -> WorkflowState:
        ...
        state["messages_window"].append(
            AIMessage(content=f"Component tests: {test_results['passed']} passed, {test_results['failed']} failed")
        )
    ...
    async def integration_tests(self, state: WorkflowState) -> WorkflowState:
        ...
        state["messages_window"].append(
            AIMessage(content=f"Integration tests: {test_results['passed']} passed, {test_results['failed']} failed")
        )
    ...
    async def _agent_analysis(self, agent: Any, agent_type: AgentType, context: dict) -> tuple[AgentType, str]:
