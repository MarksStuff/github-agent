...
def create_app() -> FastAPI:
    ...
    async def start_workflow(
        ...
        if not workflow_graph:
            raise HTTPException(
                status_code=503, detail="Workflow graph not initialized"
            )

        # Generate thread ID if not provided
        thread_id = (
            request.thread_id or f"workflow-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        )

        logger.info(
            f"Starting workflow {thread_id} for feature: {request.feature_name}"
        )
    ...
    async def resume_workflow(
        request: ResumeRequest, background_tasks: BackgroundTasks
    ):
        ...
        try:
            ...
            background_tasks.add_task(_resume_workflow_background, request.thread_id)
    ...
    async def get_workflow_status(thread_id: str):
        ...
        try:
            ...
            return {
                ...
                "quality_state": state.get("quality_state", {}).get("value")
                if hasattr(state.get("quality_state", {}), "value")
                else str(state.get("quality_state", "unknown")),
                "retry_count": state.get("retry_count", 0),
                "paused_for_review": state.get("paused_for_review", False),
                "last_message": state.get("messages_window", [])[-1]
                if state.get("messages_window")
                else None,
...
async def _resume_workflow_background(thread_id: str):
    ...
    try:
        ...
        logger.info(
            f"Background workflow resume completed: {thread_id} - {result['status']}"
        )
    except Exception as e:
