...
from langgraph_workflow.nodes import AgentNodes, GitNodes, InterruptNodes, ToolNodes
...
class WorkflowGraph:
    ...
    def __init__(
        self, repo_name: str, repo_path: str, checkpointer_path: str = "agent_state.db"
    ):
    ...
    def _add_graph_edges(self, graph: StateGraph) -> None:
        ...
        graph.add_conditional_edges(
            ...
            {"retry": "run_tests", "escalate": "quality_gate", "success": "run_linter"},
        )
        ...
        graph.add_conditional_edges(
            "check_ci",
            self._route_final,
            {"complete": END, "await_more_feedback": "await_feedback"},
        )
    ...
    def _route_after_feedback(
    ...
    ) -> Literal["incorporate_feedback", "implementation"]:
        ...
        if unaddressed:
            logger.info(
                f"Found {len(unaddressed)} unaddressed comments, incorporating feedback"
            )
    ...
    def _route_test_results(
        self, state: WorkflowState
    ) -> Literal["fix_failures", "quality_gate", "success"]:
    ...
    def _route_after_fixes(
        self, state: WorkflowState
    ) -> Literal["retry", "escalate", "success"]:
    ...
    def _route_final(
        self, state: WorkflowState
    ) -> Literal["complete", "await_more_feedback"]:
        ...
        all_passed = (
            state.get("quality_state") == QualityState.OK
            and state.get("test_results", {}).get("passed", False)
            and ci_status.get("all_passed", False)
        )
    ...
    async def run_workflow(
        self, thread_id: str, task_spec: str, feature_name: str, config: dict = None
    ) -> dict:
        ...
        try:
            ...
            async for event in self.app.astream(initial_state, config=config):
                ...
                if "messages_window" in event:
                    latest_message = (
                        event["messages_window"][-1] if event["messages_window"] else {}
                    )
                    logger.info(
                        f"Workflow progress: {latest_message.get('content', 'Step completed')}"
                    )
        ...
        try:
            ...
            async for event in self.app.astream(None, config=config):
                ...
                if "messages_window" in event:
                    ...
                    logger.info(
                        f"Resumed workflow progress: {latest_message.get('content', 'Step completed')}"
                    )
