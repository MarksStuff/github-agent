...
class StateValidator:
    ...
    def validate_phase_transition(
        current_phase: WorkflowPhase, next_phase: WorkflowPhase
    ) -> bool:
    ...
    def validate_pr_integration(state: WorkflowState) -> tuple[bool, list[str]]:
        ...
        comment_ids = {
            str(comment.get("id", "")) for comment in pr_comments if comment.get("id")
        }
    ...
    def validate_test_integration(state: WorkflowState) -> tuple[bool, list[str]]:
        ...
        if test_results:
            ...
            if passed and returncode != 0:
                issues.append(
                    "Test results inconsistent: passed=True but returncode != 0"
                )
            elif not passed and returncode == 0:
                issues.append(
                    "Test results inconsistent: passed=False but returncode == 0"
                )
    ...
    def _validate_enum_field(
        value: Any, enum_class: type, field_name: str
    ) -> Optional[str]:
    ...
    def _validate_state_consistency(state: WorkflowState) -> list[str]:
        ...
        if artifacts_index:
            ...
            invalid_paths = [
                key
                for key, path in artifacts_index.items()
    ...
    def get_validation_summary(state: WorkflowState) -> dict:
        ...
        """
        summary = {"overall_valid": True, "total_errors": 0, "checks": {}}
