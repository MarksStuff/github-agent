...
class ArtifactManager:
    ...
    def save_artifact(self, thread_id: str, artifact_type: str, filename: str, 
    ...
    def load_artifact(self, thread_id: str, artifact_type: str, filename: str) -> Optional[str]:
    ...
    def load_artifact_metadata(self, thread_id: str, artifact_type: str, filename: str) -> Optional[dict]:
        ...
        """
        metadata_path = self.get_thread_dir(thread_id) / artifact_type / f"{filename}.meta.json"
    ...
    def list_artifacts(self, thread_id: str, artifact_type: Optional[str] = None) -> list[dict]:
        ...
        search_dirs = [thread_dir / artifact_type] if artifact_type else list(thread_dir.iterdir())
        ...
        for type_dir in search_dirs:
            ...
            for artifact_file in type_dir.iterdir():
                if artifact_file.suffix == ".json" and artifact_file.name.endswith(".meta.json"):
                ...
                artifact_info = {
                    ...
                    "modified_at": datetime.fromtimestamp(artifact_file.stat().st_mtime).isoformat()
                }
                
                # Add metadata if available
                metadata = self.load_artifact_metadata(thread_id, type_dir.name, artifact_file.name)
                if metadata:
    ...
    def get_storage_stats(self) -> dict:
        ...
        for thread_dir in thread_dirs:
            ...
            for artifact_file in thread_dir.rglob("*"):
                if artifact_file.is_file() and not artifact_file.name.endswith(".meta.json"):
