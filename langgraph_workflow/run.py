...
from langgraph_workflow import MultiAgentWorkflow, WorkflowState, WorkflowPhase, AgentType, ModelRouter
    ...
    for line in lines:
        ...
        if feature_name.lower() in line.lower():
            ...
            if (line.startswith('#') or 
                line.startswith('##') or 
                (line.strip() and line[0].isdigit() and '.' in line[:5])):
...
async def interactive_mode():
    ...
    while True:
        ...
        if choice == "1":
            ...
            thread_id = input("Thread ID (optional, press Enter to auto-generate): ").strip()
            ...
            import sqlite3
            db_path = input("Database path (default: agent_state.db): ").strip() or "agent_state.db"
...
Examples:
  ...
  python run.py --interactive
    ...
    parser.add_argument("--feature-file", help="Path to file containing feature description or PRD")
    parser.add_argument("--feature-name", help="Name of specific feature within PRD (requires --feature-file)")
    parser.add_argument("--thread-id", help="Thread ID for persistence")
    parser.add_argument("--checkpoint-path", default="agent_state.db", help="SQLite checkpoint database path")
