...
def update_messages_window(state: WorkflowState, new_message: dict, max_size: int = 10) -> None:
...
def should_escalate(state: WorkflowState) -> bool:
    ...
    return any([
        ...
        state.get("quality_state") == QualityState.FAIL and state.get("retry_count", 0) >= 1
    ])