...
Example:
  python step2_create_design_document.py --pr 123
    ...
    context_file = Path(repo_path).resolve() / ".workflow" / f"context_pr_{args.pr}.json"
    ...
    if consolidated_design.exists():
        ...
        if response != 'y':
            print("Skipping design generation. Run step3_finalize_design_document.py next.")
            return 0
    ...
    try:
        ...
        if result["status"] == "success":
            ...
            print("\nNext steps:")
            print(f"1. Review the consolidated design in {repo_path}/.workflow/round_2_design/")
            print("2. Provide feedback via GitHub PR comments")
            print(f"3. Run 'python step3_finalize_design_document.py --pr {args.pr}'")
            
            return 0
        else:
            print(f"❌ Design consolidation failed: {result.get('error', 'Unknown error')}")
            return 1
