...
class InteractiveDevelopmentProcessor:
    ...
    async def run_development_process(self) -> dict:
        ...
        try:
            ...
            if not design_content:
                return {"status": "failed", "error": "No finalized design document found"}
            
            # Parse implementation tasks from the design
            tasks = await self._parse_implementation_tasks(design_content)
            if not tasks:
                return {"status": "failed", "error": "No implementation tasks found in design"}
            ...
            return {
                ...
                "tasks_completed": len([t for t in completed_tasks if t["status"] == "success"]),
    ...
    async def _load_finalized_design(self) -> str | None:
        ...
        consolidated_path = self.workflow_dir / "round_2_design" / "consolidated_design.md"
    ...
    async def _parse_implementation_tasks(self, design_content: str) -> List[Dict[str, Any]]:
...
Only return the JSON array, no other text.
    ...
    async def _execute_four_part_cycle(self, task: Dict[str, Any], task_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            print("\n--- Part 2: Multi-Agent Review and Refinement ---")
            part2_result = await self._part2_multi_agent_review(task, task_number, part1_result)
            ...
            return {
                ...
                "files_modified": part1_result.get("files_created", []) + part2_result.get("files_modified", []) + part4_result.get("files_modified", [])
            }
    ...
    async def _part1_interactive_coding(self, task: Dict[str, Any], task_number: int) -> Dict[str, Any]:
        ...
        design_finalized_path = self.workflow_dir / "round_3_design" / "finalized_design.md"
        design_consolidated_path = self.workflow_dir / "round_2_design" / "consolidated_design.md"
...
BEGIN: Read the codebase analysis document now.
        ...
        print(f"You will now enter an interactive coding session with the developer agent.")
        print(f"The agent knows about the task and will help you implement it.")
        print("="*60)
        ...
        try:
            ...
            print(f"The developer persona will be loaded with the task context.")
            ...
            print(f"Task: {task['title']}")
            print(f"\nINSTRUCTIONS:")
            print(f"1. Claude chat will open")
            print(f"2. Copy and paste the context from: {context_file}")
            print(f"3. Work with Claude to implement the task")
            print(f"4. Exit with Ctrl+D when done")
            print("-" * 60)
            ...
            try:
                ...
                print(f"Visit: https://docs.anthropic.com/en/docs/claude-cli")
    ...
    async def _part2_multi_agent_review(self, task: Dict[str, Any], task_number: int, part1_result: Dict[str, Any]) -> Dict[str, Any]:
        """Part 2: Multi-agent review and refinement."""
        ...
        architect_review = await self._get_agent_review("architect", task, implemented_files)
        reviews["architect"] = architect_review
        
        # Senior Engineer Review  
        print("  - Senior Engineer reviewing...")
        senior_review = await self._get_agent_review("senior_engineer", task, implemented_files)
        ...
        refinement_result = await self._apply_review_refinements(task, task_number, reviews)
        
        # Commit the refined code
        await self._commit_part2_refinements(task, task_number)
        
        print(f"✅ Part 2 completed - Multi-agent review and refinement")
        return {
            "status": "success",
            "reviews": reviews,
    ...
    async def _part3_human_review_break(self, task: Dict[str, Any], task_number: int) -> Dict[str, Any]:
        ...
        print("\nPress Enter when you have finished adding PR comments (or press Enter to skip): ", end="")
        
        # Wait for human input
        user_input = input().strip()
        
        print(f"✅ Part 3 completed - Human review break")
        return {"status": "success", "human_input": user_input}
    
    async def _part4_pr_comment_integration(self, task: Dict[str, Any], task_number: int) -> Dict[str, Any]:
        """Part 4: PR comment integration."""
        ...
        print(f"✅ Part 4 completed - PR comment integration")
        return {
            "status": "success",
            "comments_processed": len(comments),
            "files_modified": [r.get("file_modified") for r in integration_results if r.get("file_modified")],
            "integration_results": integration_results
        }
    
    # Helper methods for the 4-part cycle
    async def _get_agent_review(self, agent_type: str, task: Dict[str, Any], implemented_files: List[str]) -> Dict[str, Any]:
...
Provide specific, actionable feedback.
    ...
    async def _apply_review_refinements(self, task: Dict[str, Any], task_number: int, reviews: Dict[str, Any]) -> Dict[str, Any]:
...
Apply the refinements now.
        ...
        return {
            ...
            "files_modified": refinement_result.get("files_created", []) if refinement_result else [],
            "status": refinement_result.get("status", "failed") if refinement_result else "failed"
        }
    ...
    async def _integrate_pr_comment(self, comment: Dict[str, Any], task: Dict[str, Any], task_number: int) -> Dict[str, Any]:
...
Focus on addressing the specific concern raised in the comment.
        ...
        return {
            "status": "success" if integration_result and integration_result.get("status") == "success" else "failed",
            "comment_id": comment.get("id"),
            "file_modified": file_path,
            "changes_made": bool(integration_result and integration_result.get("status") == "success")
        }
    
    async def _reply_to_integrated_comments(self, comments: List[Dict[str, Any]], task: Dict[str, Any]):
...
The code has been updated to address your concerns. Please review the latest commit."""
    ...
    async def _commit_part1_implementation(self, task: Dict[str, Any], task_number: int):
...
Example:
  python step4_implementation.py --pr 123
    ...
    setup_logging(log_level=args.log_level, log_file=log_dir / "step4_implementation.log")
    ...
    if result["status"] == "success":
        ...
        print(f"  - All tasks went through 4-part development cycle")
        
        print(f"\nNext steps:")
        print(f"1. Review all commits in the PR to see the development evolution")
        print(f"2. Run tests to verify implementation")
        print(f"3. Make any final adjustments if needed")
        print(f"4. Merge the PR when ready")
        return 0
    else:
        print(f"❌ Development process failed: {result.get('error')}")
        print(f"\nSome tasks may have been partially completed.")
        print(f"Check the PR and logs for details.")
        return 1
