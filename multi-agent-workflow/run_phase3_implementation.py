...
class Phase3ImplementationRunner:
    ...
    def __init__(self, pr_number: Optional[int] = None):
        ...
        default_repo_path = (
            str(Path.cwd().parent)
            if Path.cwd().name == "multi-agent-workflow"
            else str(Path.cwd())
        )
        ...
        logger.info("Initialized Phase 3 runner (auto-detecting repo from git remote)")
    ...
    async def run_complete_workflow(self) -> dict[str, Any]:
        ...
        try:
                ...
                await self._generate_implementation_summary(
                    {
                        ...
                        "files_created_count": result[
                            "files_created"
                        ],  # Store the count separately
                        "tests_added": result.get("test_results", {}).get(
                            "test_files", []
                        ),
                        "errors": [],
                        "success": True,
            ...
            return {"status": "failed", "error": error_msg, "pr_number": self.pr_number}
    ...
    async def _load_design_document(self) -> Optional[str]:
        ...
        try:
            ...
            possible_paths = [
                ...
                / "agent_collaboration_design.md",
                Path(self.repo_path)
                / "github-agent"
                / "multi-agent-workflow"
                / "agent_collaboration_design.md",
                Path(__file__).parent
                / "agent_collaboration_design.md",  # Same directory as this script
            ]
            ...
            if not design_doc_path:
                logger.warning(
                    f"Design document not found in any of these locations: {[str(p) for p in possible_paths]}"
                )
                return None
    ...
    async def _extract_implementation_tasks(
        self, design_document: str
    ) -> list[dict[str, Any]]:
        ...
        if not tasks:
            ...
            logger.info(
                "📋 Phase 3 implementation is COMPLETE - no additional coding needed"
            )
    ...
    async def _component_exists(self, component: str) -> bool:
        ...
        if "code generation" in component_lower:
            ...
            return (
                Path(self.repo_path) / "multi-agent-workflow" / "coding_personas.py"
            ).exists()
    ...
    async def _execute_implementation_task(
        self, task: dict[str, Any], cycle_number: int
    ) -> dict[str, Any]:
        ...
        try:
            ...
            result = await self._attempt_real_implementation(
                task, persona, implementation_prompt
            )
    ...
    async def _execute_verification_task(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            agent_interface_exists = (
                Path(self.repo_path) / "multi-agent-workflow" / "agent_interface.py"
            ).exists()
            tests_exist = (Path(self.repo_path) / "tests").exists()

            all_verified = (
                personas_exist
                and orchestrator_exists
                and agent_interface_exists
                and tests_exist
            )
    ...
    async def _attempt_real_implementation(
        self, task: dict[str, Any], persona, prompt: str
    ) -> dict[str, Any]:
        ...
        if "code generation" in component.lower():
            ...
            if (
                ...
                and "analyze_feature" in workflow_file.read_text()
            ):
                return {
                    "success": True,
...
"""
    ...
    async def _generate_implementation_summary(self, results: dict[str, Any]) -> None:
        ...
        if results.get("phase_complete", False):
            ...
            logger.info(
                f"Successful Implementations: {results['implementations_created']}"
            )
            logger.info(
                f"Files Created: {results.get('files_created_count', len(results.get('files_modified', [])))}"
            )
            ...
            success_rate = (
                results["implementations_created"] / max(results["cycles_completed"], 1)
            ) * 100
